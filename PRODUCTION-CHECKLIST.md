...
## Phase 0 additions (inquiry reliability — must be true before launch)

- [ ] `MAIL_SERVER`, `MAIL_PORT`, `MAIL_USE_TLS`, `MAIL_USERNAME`, `MAIL_PASSWORD` set in Render. For the company Gmail: `smtp.gmail.com`, `587`, `true`, `royalexoticfarms@gmail.com`, and a Google **App password** (requires 2-Step Verification).
- [ ] `SUBMISSION_NOTIFY_EMAIL` is an inbox someone reads daily (defaults to `ORG_EMAIL`).
- [ ] `MAIL_SUPPRESS_SEND` is `false` (Production already forces this).
- [ ] Persistent database: set `DATABASE_URL` to Postgres. SQLite on Render is erased on every deploy/restart.
- [ ] Remove any placeholder values left in Render env (`ORG_PHONE=+91 00000 00000`, `ORG_WHATSAPP=910000000000`, `ORG_ADDRESS=India`). The app now ignores them, but an env value always overrides the verified default, so a placeholder there means the field is hidden.
- [ ] `SITE_URL` set to the live origin (no trailing slash).
- [ ] After deploy, read the Render log: no `PRODUCTION CONFIG:` error lines.
- [ ] Submit one real test inquiry and confirm (a) the email arrives, (b) the row exists in `submissions`.
- [ ] Search the logs for `LEAD NOT EMAILED` periodically — each one is a lead that must be followed up by hand.
